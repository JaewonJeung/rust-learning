...
impl Frame {
    ...
    pub fn check(src: &mut Cursor<&[u8]>) -> Result<(), Error> {
        ...
        if src.remaining() > 0 {
            dlog!(
                "Frame::check - starting at pos={}, first byte={:?}",
                pos,
                src.chunk()[0]
            );
        }
        ...
        match get_u8(src)? {
            ...
            b'*' => {
                ...
                dlog!("Frame::check - array length={}", len);
    ...
    pub fn parse(src: &mut Cursor<&[u8]>) -> Result<Frame, Error> {
        ...
        dlog!(
            "Frame::parse - at pos={} remaining={} first={:?}",
            start,
            src.remaining(),
            if src.remaining() > 0 {
                Some(src.chunk()[0])
        ...
        match get_u8(src)? {
            ...
            b'*' => {
                let len = get_decimal(src)?.try_into()?;
                dlog!("Frame::parse - parsing array len={}", len);
...
fn get_decimal(src: &mut Cursor<&[u8]>) -> Result<u64, Error> {
    ...
    match atoi::<u64>(line) {
        ...
        None => Err(Error::Other(anyhow::anyhow!(
            "protocol error; invalid frame format"
        ))),
    }
}
...
impl From<&str> for Error {
    fn from(src: &str) -> Error {
        Error::Other(anyhow::anyhow!(src.to_string()))
    }
}

impl From<FromUtf8Error> for Error {
    fn from(_src: FromUtf8Error) -> Error {
        Error::Other(anyhow::anyhow!("protocol error; invalid frame format"))
    }
}
